...
report_tools() {
  tools[mrx]='MRX|subfinder findomain-linux assetfinder|for log in "$logdir/"{assetfinder,findomain-linux,sub{finder}}.log; do > "$log"; done; sleep 10; subfinder -d "$domain" -silent -o "$logdir/subfinder.log"; sleep 10; assetfinder -subs-only "$domain" > "$logdir/assetfinder.log"; sort -u "$logdir/"{assetfinder,findomain-linux,sub{finder}}.log -o "$logfile"; httpx -silent < "$logfile" > "$logdir/${dtreport}httpx.log"'
