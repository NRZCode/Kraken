...
init_install() {
  ...
  if [[ ! -f $HOME/.local/.arno_init_install_successful ]]; then
    ...
    apt -y install kali-desktop-gnome gospider
    sudo $SUDO_OPT pip3 install --upgrade pip
    ...
    sudo $SUDO_OPT pip install one-lin3r bluto dnspython requests win_unicode_console colorama netaddr aiodnsbrute webscreenshot pipx
