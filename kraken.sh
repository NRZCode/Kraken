...
report_tools() {
  ...
  tools[email-search]='email-search|EmailHarvester emailfinder holehe|EmailHarvester -d $target_domain -e all -s $logdir/${dtreport}emailharvester.log; emailfinder -d $target_domain > $logdir/${dtreport}emailfinder.log; while read mail; do holehe --no-clear --only-used "$mail"; done < $logdir/${dtreport}emailharvester.log > "$logfile"'
}
